...
*/
t_calced_walls	check_intersect_with_all_wall_rend(t_data *d, t_point pos,
					t_rot rot, t_rot look_rot);
...
void			move_attempt(t_data *d, t_point *pos, float speed,
					t_rot look_rot);
void			move_with_collide(t_data *d, t_obj *obj, t_rot rot,
					float speed);
void			move_with_collide_player(t_data *d, t_point *pos, t_rot rot,
					float speed);
void			gravity(t_data *d);
int				check_collide(t_data *d, t_ray *res, float z_pos,
					int *will_collide);
...
t_calced_walls	check_inter_with_wall(t_wall wall, t_rot rot, t_point pos,
					t_rot look_rot);
void			sort_walls_by_dist(t_data *d, t_point pos, t_rot current_angle,
					t_calced_walls *sorted_walls);
void			sort_walls_by_dist_player(t_data *d, t_point pos,
					t_rot current_angle, t_calced_walls *sorted_walls);
...
void			sort_perp_walls_dist(t_map_data *map, t_point pos,
					t_calced_walls *sorted_walls);
t_ray			check_inter_with_obj(t_obj *obj, t_rot rot, t_point pos,
					t_rot look_rot);
...
void			init_perp_points(t_point *p1, t_point *p2, t_point pos,
					t_rot look_rot);
...
** render_raycast.C
*/
void			raycast_screen(t_data *d, t_range screen_x, float start_angle,
					float step);
void			*raycast_thread(void *data);
void			raycast_thread_init(t_data *d);
void			raycast_all_screen(t_data *d, t_map_data *map);
void			raycast_screen2(t_data *d, t_range screen_x, float start_angle,
					float step);
...
void			move_wall(t_data *d, t_wall *wall, t_rot look_rot,
					float speed);
...
t_proj_point	point_x_on_screen(t_data *d, t_point point, float rot,
					t_point pos);
...
*/
void			draw_floor(t_data *d, SDL_Surface *text);
t_draw			calc_floor_draw_range(t_data *d, t_ray ray1,
					float dist2, int x);
t_draw			calc_floor_draw_range_end(t_data *d, float dist1, t_room *room,
					int x);

void			draw_floor_line(t_data *d, t_range draw, int x,
					int room_id);
void			print_floor_slice(t_data *d, t_floor *fl, t_draw draw,
					int text_id);
...
t_range			calc_prop_draw_range(t_data *d, float dist, float height,
					float size);
...
** render_skybox.c
*/
void			print_skybox(t_data *d, t_img skybox, float rot,
					unsigned int *screen);
