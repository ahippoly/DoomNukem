...
void	mouseclick_event(t_enval *env)
{
	...
	&& env->event.button.button == SDL_BUTTON_LEFT)
	{
		...
		{
			if (env->game.pc.arsenal[env->game.pc.equip.current].attack.type
				== WEAPON_TYPE_CHARGED && env->game.pc.equip.weapon_state
				!= WEAPON_STATE_FIRING)
		...
		&& env->event.button.button == SDL_BUTTON_LEFT)
		env->sdl.key[LMOUSE] = env->game.pc.equip.weapon_state
		!= WEAPON_STATE_READY ? 1 : 0;
}
