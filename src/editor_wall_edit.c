...
void check_mouse_in_walls(t_env *env)
{
    ...
    {
        p1 = env->wall_list[i].p1;
        p2 = env->wall_list[i].p2;
        if (is_cursor_in_wall(env->mouse, p1, p2))
            octant(mult_sdl_point(p1, TILE_SIZE), mult_sdl_point(p2, TILE_SIZE), env->p_grid, 0xFF0000FF, GRID_SIZE_X);
        i++;
    }
}