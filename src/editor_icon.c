...
/*   Updated: 2020/07/07 19:15:57 by ahippoly         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			print_icon(t_env *env, t_icon icon)
{
	...
	if (icon.pos.x > 0)
	{
		tmp = set_sdl_rect(icon.pos.x * TILE_SIZE - icon.size.w / 2 +
		env->map_move.x, icon.pos.y * TILE_SIZE - icon.size.h / 2 +
		env->map_move.y, icon.size.w, icon.size.h);
		if ((SDL_RenderCopy(env->rend, env->img_list[icon.id_ref],
			NULL, &tmp)) < 0)
...
void			print_mouse_icon(t_env *env)
{
	if (env->mouse_icon.id_ref != -1)
	{
		...
		env->mouse_icon.pos.x = (float)(env->mouse.x - GRID_POS_X
			- env->map_move.x) / TILE_SIZE;
		env->mouse_icon.pos.y = (float)(env->mouse.y - GRID_POS_Y
			- env->map_move.y) / TILE_SIZE;
...
void			print_player_spawn(t_env *env)
{
	if (env->player_spawn.x > 0.001)
	{
		if ((SDL_SetRenderTarget(env->rend, env->editor_grid)) < 0)
			exit_editor(env, "error : failed to render target");
		print_icon(env, create_icon(env->player_spawn,
			create_t_size(ICON_PLAYER_SIZE, ICON_PLAYER_SIZE), 0));
		if ((SDL_SetRenderTarget(env->rend, NULL)) < 0)
			exit_editor(env, "error : failed to render target");
	}
}
