...
void draw_sub_rectangle(t_point point[4], char *pixels, t_text *text)
{
	...
	line[0] = mem_octant(point[0], point[2], &drawed_length[0], 2);
	line[1] = mem_octant(point[1], point[3], &drawed_length[1], 2);
...
{
	unsigned char *text_pix;

	text_pix = (unsigned char *)text->pixels;
...
void draw_scanline(t_point start, t_point end, unsigned char *p_tab, t_text *text)
{
	int i;
	int scan_length;
	t_pos text_step;
...
void create_sub_triangle(t_point original[3], t_point sub[3], int is_topside)
{
	...
	else
	{
		...
		sub[1] = original[1];
		sub[2] = original[0];
		sub[0].zdist = original[2].zdist;
		sub[2].zdist = original[0].zdist;
	}
