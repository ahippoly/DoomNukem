...
#include "wolf3d.h"
/*
**Do not forget "i++" at the end of the main loop
*/
...
t_enval			*ray_calc(t_enval *env, int i)
{
	...
	env->ray.deltadist.x = sqrt(1 + (env->ray.dir.y * env->ray.dir.y) / (env->ray.dir.x * env->ray.dir.x));
	env->ray.deltadist.y = sqrt(1 + (env->ray.dir.x * env->ray.dir.x) / (env->ray.dir.y * env->ray.dir.y));
...
static t_enval	*perp_height(t_enval *env)
{
	if (env->ray.wallside == 0)
		env->ray.perpwalldist = ft_abs((env->ray.mapos.x - env->player.pos.x + (1 - env->ray.step.x) / 2) / env->ray.dir.x);
	else
		env->ray.perpwalldist = ft_abs((env->ray.mapos.y - env->player.pos.y + (1 - env->ray.step.y) / 2) / env->ray.dir.y);
